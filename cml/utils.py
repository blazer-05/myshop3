...
class ImportManager(object):
    ...
    def _get_tree(self):
        ...
        it = ET.iterparse(StringIO(open(self.file_path).read()))
        for _, el in it:
    ...
    def _parse_products(self, current_element):
        for product_element in current_element.findall(u'Товары/Товар'):
            ...
            brand_element = product_element.find(u'Изготовитель')
            if brand_element is not None:
                brand_item = Brand(brand_element)
                brand_item.id = self._get_cleaned_text(brand_element.find(u'Ид'))
                brand_item.name = self._get_cleaned_text(brand_element.find(u'Наименование'))
                product_item.brand_id = brand_item.id
                self.item_processor.process_item(brand_item)
